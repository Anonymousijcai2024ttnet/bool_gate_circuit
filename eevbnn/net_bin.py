...
class SeqBinModelHelper:
    ...
    def get_sparsity_stat(self):
        ...
        for i in self.features:
            ...
            if isinstance(i, (Classifier_scale)):
                with torch.no_grad():
                    wb = i.lin1.weight_bin
                    nz = int((wb.abs() < 1e-4).to(dtype=torch.int32).sum())
                    cur_nr = wb.numel()
                tot += cur_nr
                nr_zero += nz
                ...
                parts.append(nz / cur_nr)
                with torch.no_grad():
                    wb = i.linvf.weight_bin
                    nz = int((wb.abs() < 1e-4).to(dtype=torch.int32).sum())
                    cur_nr = wb.numel()
                tot += cur_nr
                nr_zero += nz
                parts.append(nz / cur_nr)
            if isinstance(i, (Classifier_scale_all)):
                with torch.no_grad():
                    wb = i.lin1.weight_bin
                    nz = int((wb.abs() < 1e-4).to(dtype=torch.int32).sum())
                    cur_nr = wb.numel()
...
class Classifier_scale(SeqBinModelHelper, nn.Module, ModelHelper):
    '''Depthwise conv + Pointwise conv'''
    def __init__(self, fcsize, linpos, out_planes=10, inter=50, non_lin=400):
        super(Classifier_scale, self).__init__()
        #lin = BinLinearPos
        #linpp =BinLinearPosv2
        wb = g_weight_binarizer
        self.non_lin = non_lin
        self.lin1 = linpos(wb, non_lin, inter)
        self.BN1 =  setattr_inplace(BatchNormStatsCallbak(
                self, inter,
                use_scalar_scale=g_use_scalar_scale_last_layer),
            'bias_regularizer_coeff', 0)
        self.lin2 = linpos(wb,fcsize-non_lin, inter)
        self.BN2 = setattr_inplace(BatchNormStatsCallbak(
                self, inter,
                use_scalar_scale=g_use_scalar_scale_last_layer),
            'bias_regularizer_coeff', 0)
        self.linvf = linpos(wb,inter+inter, out_planes)
        self.BNvf = setattr_inplace(BatchNormStatsCallbak(
                self, out_planes,
                use_scalar_scale=g_use_scalar_scale_last_layer),
            'bias_regularizer_coeff', 0)
...
class Classifier_scale_all(SeqBinModelHelper, nn.Module, ModelHelper):
    '''Depthwise conv + Pointwise conv'''
    def __init__(self, fcsize, linpos, out_planes=10, inter=100):
        super(Classifier_scale_all, self).__init__()
        #lin = BinLinearPos
        #linpp =BinLinearPosv2
        wb = g_weight_binarizer
        #self.non_lin = non_lin
        self.lin1 = linpos(wb, fcsize, inter)
        self.BN1 =  setattr_inplace(BatchNormStatsCallbak(
                self, inter,
                use_scalar_scale=g_use_scalar_scale_last_layer),
            'bias_regularizer_coeff', 0)
        ...
        #        use_scalar_scale=g_use_scalar_scale_last_layer),
        #    'bias_regularizer_coeff', 0)
        self.linvf = linpos(wb,inter, out_planes)
        self.BNvf = setattr_inplace(BatchNormStatsCallbak(
                self, out_planes,
                use_scalar_scale=g_use_scalar_scale_last_layer),
            'bias_regularizer_coeff', 0)
    ...
    def forward(self, x):
        ...
        x1 = self.poly(x1)
        #xf = torch.cat((x1,x2), axis=1)
        return self.BNvf(self.linvf(x1))
