from config.config import transform_input_filters, transform_input_filters2, two_args_str_float, transform_input_thr
...
class WeightClip:
    ...
    def __init__(self, model):
        ...
        for i in model.features:
            if isinstance(i, (TT_net.BinConv2d, TT_net.BinLinear, TT_net.BinLinearPos)):
                weights.append(i.weight)
                if (mask := i._parameters.get('weight_mask')) is not None:
                    weights.append(mask)
            elif isinstance(i, (TT_net.Block_TT)):
                weights.append(i.conv1.weight)
                weights.append(i.conv2.weight)
                weights.append(i.bn1.weight)
                weights.append(i.bn2.weight)
...
class GradScaleChooser:
    ...
    def __init__(self, args, device, model, trainloader):
        ...
        self._scale_params = [i.grad_scale for i in model.features
                              if isinstance(i, TT_net.Binarize01Act)]
    ...
    def train_epoch(self, epoch):
        ...
        if (spar_stat := self._net.get_sparsity_stat()) is not None:
            ...
            logmsg(f'sparsity@{epoch}: {spar_parts} ; '
                   f'{spar_nz}/{spar_tot+0.001}={spar_nz/(spar_tot+0.001)*100:.2f}%')
...
def main():
    ...
    parser.add_argument("--groups", type=transform_input_filters)
    parser.add_argument("--filters", type=transform_input_filters)
    parser.add_argument("--kernelsizes", type=transform_input_filters)
    parser.add_argument("--amplifications", type=transform_input_filters)
    parser.add_argument("--strides", type=transform_input_filters)
    parser.add_argument("--thr_bin_act", type=transform_input_thr)
    parser.add_argument("--thr_bin_act_test", type=transform_input_thr)

    parser.add_argument("--preprocessing_CNN", type=transform_input_filters)
    parser.add_argument("--paddings", type=transform_input_filters)
    parser.add_argument("--first_layer")#, type=transform_input_filters)
    parser.add_argument("--last_layer")#, type=transform_input_filters)

    parser.add_argument("--Blocks_filters_output", type=transform_input_filters)
    parser.add_argument("--Blocks_amplifications", type=transform_input_filters)
    parser.add_argument("--Blocks_strides", type=transform_input_filters)
    parser.add_argument("--type_first_layer_block")#, type=transform_input_filters)

    parser.add_argument("--kernel_size_per_block", type=transform_input_filters)
    parser.add_argument("--groups_per_block", type=transform_input_filters)
    parser.add_argument("--padding_per_block", type=transform_input_filters)
    parser.add_argument("--kernel_size_per_block_multihead", type=transform_input_filters)

    parser.add_argument("--groups_per_block_multihead", type=transform_input_filters)
    parser.add_argument("--paddings_per_block_multihead", type=transform_input_filters)
    parser.add_argument("--type_blocks", type=transform_input_filters2)

    parser.add_argument("--data_augmentation", type=str)

    parser.add_argument("--dataset", type=str)

    parser.add_argument("--g_remove_last_bn", type=str)
    ...
    net_class = getattr(TT_net, args.net)
